# -*- coding: utf-8 -*-
"""
Created on Thu Apr  1 02:14:12 2021
...
class main:
    ...
    def registration(self):
        root.destroy()
        from subprocess import call
        call(["python", "Numberplate_registration.py"])
 ...
 # , relwidth=1, relheight=1)
    ...
    # Draw Widgets
    def widgets(self):
        self.head = tk.Label(self.master, text='Welcome To Login', background="salmon", font=('Times New Roman', 20),
                             pady=20)
        self.head.pack()
        ...
        # self.head.pack()
        self.logf = tk.Frame(self.master, padx=50, pady=20, background="salmon")
        tk.Label(self.logf, text='Username: ', background="salmon", font=("Times New Roman", 20), pady=5,
                 padx=5).grid(sticky=tk.W)
        
        tk.Entry(self.logf, textvariable=self.username, bd=5, background="white", font=('', 15)).grid(row=0, column=1)
        tk.Label(self.logf, text='Password: ', background="salmon", font=("Times New Roman", 20), pady=5,
                 padx=5).grid(sticky=tk.W)
